...
void StereoEPVO::run() {
    ...
    for (n_cur = n_start + 1; n_cur < n_final; n_cur++) {
        ...
        for (int lvl = params->pyr.max_lvl - 1; lvl > -1; --lvl) {
            ...
            //motracker->calcResidual(T_ck, lvl, is_cached, huber_scaler_edge, huber_scaler_point);
            motracker_sse->calcResidual(T_ck, lvl, is_cached, huber_scaler_edge, huber_scaler_point);
            if (is_cached == false) is_cached = true;
            while ((motracker_sse->n_overthres_point / motracker_sse->n_valid_pointpatch) > 0.6 && huber_scaler_point < 5) {
                float saturated_ratio = (motracker_sse->n_overthres_point / motracker_sse->n_valid_pointpatch);
            ...
            {
                tic();
                //motracker->fillHJr(T_ck, lvl, is_cached, huber_scaler_edge, huber_scaler_point); // render A and b
                //motracker->solveLMstep(delta_step, lambda);     // solve A*x = b

                motracker_sse->warpAndMatchAndCheckAndAffineBrightAndGenerateEdgeBuffers(T_ck, lvl, is_cached, alpha, beta);
                motracker_sse->calcEdgeResidualAndWeightSSE(huber_scaler_edge);
                motracker_sse->calcEdgeHessianAndJacobianSSE(lvl);
                motracker_sse->warpPointPatchAndGeneratePointPatchBuffers(T_ck, lvl);
                motracker_sse->calcPointPatchResidualAndWeightSSE(huber_scaler_point, alpha, beta);
                motracker_sse->calcPointPatchHessianAndJacobianSSE(lvl, alpha, beta);
                motracker_sse->solveLevenbergMarquardtStepSSE(delta_step, lambda);

                Eigen::Matrix4d delta_T;
                Lie::se3Exp(delta_step.block<6, 1>(0, 0), delta_T);
                Eigen::Matrix4d T_ck_new = delta_T*T_ck;
                //motracker->calcResidual(T_ck_new, lvl, is_cached, huber_scaler_edge, huber_scaler_point);
                motracker_sse->calcResidual(T_ck_new, lvl, is_cached, huber_scaler_edge, huber_scaler_point);

                // decide whether to accept this step
                //res_new = motracker->err;
                res_new = motracker_sse->err;
                bool is_decresing = res_new < res_old;
                if (iter) {
                    if (is_decresing) { // good convergence! and iter > 0
                                        // decrease lambda, update! 
                ...
                if (is_decresing) {
                    cout << "     lv" << lvl << ",itr:" << iter << ",lam:" << lambda << " // err: " << motracker_sse->err
                        << ", err_e: " << motracker_sse->err_e << ", err_p: " << motracker_sse->err_ppat
                    ...
                    // cout << "                 !! increasing Energy!! skip update" << endl;
                }
            }
