...
void Lie::r2a(const Eigen::Matrix3d& R, double& r, double& p, double& y) 
{
    r = atan2(R(2, 1), R(2, 2));
    p = asin(-R(2, 0));
    y = atan2(R(1, 0), R(0, 0));
...
void Lie::r2a(const Eigen::Matrix3f& R, float& r, float& p, float& y)
{
    r = atan2(R(2, 1), R(2, 2));
    p = asin(-R(2, 0));
    y = atan2(R(1, 0), R(0, 0));
...
void Lie::se3Exp(const Eigen::Matrix<float, 6, 1>& xi_, Eigen::Matrix4d& T_)
{
...
void Lie::se3Exp(const Eigen::MatrixXd& xi_, Eigen::MatrixXd& T_)
{
	// initialize variables
	double theta = 0.0;
	Eigen::Vector3d v, w;
...
void Lie::SE3Log(const Eigen::MatrixXf& T_, Eigen::MatrixXf& xi_)
{
	float theta = 0.0f;
	float A, B;
	Eigen::Matrix3f wx, R;
...
void Lie::SE3Log(const Eigen::MatrixXd& T_, Eigen::MatrixXd& xi_)
{
	double theta = 0.0;
	double A, B;
	Eigen::Matrix3d wx, R;
...
void Lie::a2r(const float& r, const float& p, const float& y, Eigen::Matrix3f& R) {// r,p,y are defined on the radian domain.
...
Eigen::Matrix3d Lie::hat(const Eigen::Vector3d& v)
{
    Eigen::Matrix3d res;
    res << 0, -v(2), v(1),
        v(2), 0, -v(0),
